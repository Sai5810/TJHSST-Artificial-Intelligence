...
def backtrackKS(domain, assignment, neighbors, sums, keyind, valind, cursum):
    ...
    for val in domain[var]:
        ...
        if valind + 1 < len(sums[keyind][0]):
            ...
            for j in neighbors[var]:
                if j in d1 and str(val) in d1[j]:
                    d1[j] = d1[j].replace(str(val), '')
            result = backtrackKS(d1, a1, neighbors, sums, keyind, valind + 1, cursum + val)
            ...
            for j in neighbors[var]:
                if j in d1 and str(val) in d1[j]:
                    d1[j] = d1[j].replace(str(val), '')
            result = backtrackKS(d1, a1, neighbors, sums, keyind + 1, 0, 0)
...
def main():
    ...
    sol = backtrackKS({i: "123456789" for i in range(81)}, [0] * 81, neighbors, sums, 0, 0, 0)
    if sol is None:
        print("No Solution")
    else:
