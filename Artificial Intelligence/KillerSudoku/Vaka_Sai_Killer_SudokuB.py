...
def backtrackKS(assignment, neighbors, sums, keyind, valind, cursum):
    var = sums[keyind][0][valind]
    for val in range(1, 10):
        if isValid(neighbors, assignment, var, val):
            a1 = assignment.copy()
            a1[var] = val
            if valind + 1 < len(sums[keyind][0]):
                result = backtrackKS(a1, neighbors, sums, keyind, valind + 1, cursum + val)
            elif cursum + val != sums[keyind][1]:
                continue
            elif keyind + 1 >= len(sums):
                assignment[var] = val
                return assignment
            else:
                result = backtrackKS(a1, neighbors, sums, keyind + 1, 0, 0)
        ...
        while line := file.readline().rstrip():
            ...
            for i in sums[-1][0]:
                for j in sums[-1][0]:
                    if j != i:
                        neighbors[i].add(j)
    mean = totsum / len(sums)
    sums.sort(key=lambda x: x[1])
    # sums.sort(reverse=True, key=lambda x: abs(x[1] - mean))
    sol = backtrackKS([0] * 81, neighbors, sums, 0, 0, 0)
    if sol is None:
        print("No Solution")
    else:
