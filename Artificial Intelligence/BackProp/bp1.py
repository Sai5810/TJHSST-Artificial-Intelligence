...
def bp(layer_ct, targets, x_nodes, weights, act_func, alpha):
    ...
    print(e_nodes, e_weights, weights)
    for idx, _ in enumerate(layer_ct[1:-1]):
        sl = []
        for e in e_nodes[idx + 1]:
            for x in x_nodes[-idx - 3]:
                sl.append(x * e)
        e_weights.append(sl)
        print(weights[-idx - 2][0::(len(weights[-idx - 2]) // len(e_nodes[-1]))])
        e_nodes.append(
            [dot(weights[-idx - 2][idx2::(len(weights[-idx - 2]) // len(e_nodes[-1]))], e_nodes[-1]) * activ_der(
                act_func, x)
            ...
            if prev_err - err < .0001:
                ...
                while err > num_weights / 13:
                    ...
                    for inp, target in zip(inputs, targets):
                        ...
                        err += sum((t - n) ** 2 for t, n in zip(target, nodes[-1])) / 2
            prev_err = err
        # inp, target = list(zip(inputs, targets))[random.randint(0, len(inputs) - 1)]
