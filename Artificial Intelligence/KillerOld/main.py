...
def solve(puzzle, neighbors):
    ...
    return backtrackR(list(puzzle), neighbors, domain)


def backtrackR(assignment, neighbors, domain):
    try:
    ...
    for val in domain[var]:
        ...
        result = backtrackR(a1, neighbors, d1)
        if result is not None:
            assignment = a1.copy()
            domain = d1.copy()
            return result
...
def gen(neighbors):
    puzzle = [int(i) for i in solve('.' * 81, neighbors)]
    vis = set()
    out = {}
    for i in range(81):
        if i not in vis:
            ...
            for j in range(random.randrange(10)):
                pos = []
                if curind - 1 >= 0 and (curind - 1) // 9 == curind // 9 and (curind - 1) not in vis:
                    pos.append(curind - 1)
                if curind + 1 < 81 and (curind + 1) // 9 == curind // 9 and (curind + 1) not in vis:
                    pos.append(curind + 1)
                if curind - 9 >= 0 and (curind - 9) not in vis:
                    pos.append(curind - 9)
                if curind + 9 < 81 and (curind + 9) not in vis:
                    pos.append(curind + 9)
                if not pos:
                    break
                curind = random.choice(pos)
                vis.add(curind)
                csum += puzzle[curind]
                cval.append(curind)
            out[csum] = cval
